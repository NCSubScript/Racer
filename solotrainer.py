...
class Genetic():
    ...
    def backpropagation(self, z, y, i):
        ...
        cost = sum(self.computeLoss(y, z[len(z) - 1][0]))
        
        results = []
        layer = 0
        brain = copy.deepcopy(self.structure)
        for l in brain:
            results.append([])
            results[layer] = {"weights": [], "biases": []}
            for wr in range(len(l["weights"])):
                results[layer]["weights"].append([])
                for w in range(len(l["weights"][wr])):
                    l["weights"][wr][w] += 0.01
                    output = self.forward(i, brain)
                    results[layer]["weights"][wr].append(cost - sum(self.computeLoss(y, self.results[len(z) - 1][0])))
                    l["weights"][wr][w] -= 0.01

            results[layer]["biases"].append([])
            for b in range(len(l["biases"][0])):
                l["biases"][0][b] += 0.01
                output = self.forward(i, brain)
                results[layer]["biases"][0].append(cost - sum(self.computeLoss(y, self.results[len(z) - 1][0])))
                l["biases"][0][b] -= 0.01
            layer += 1
        self.walk(results, cost)

    def walk(self, change, cost):
        newBrain = []
        mul = 1
        if cost < 0:
            mul = -1
        for l in range(len(change)):
            ...
            for wr in range(len(change[l]["weights"])):
                newBrain[l]["weights"].append([])
                for w in range(len(change[l]["weights"][wr])):
                    newBrain[l]["weights"][wr].append(self.structure[l]["weights"][wr][w] + (mul * (change[l]["weights"][wr][w])))
            ...
            for b in range(len(change[l]["biases"][0])):
                newBrain[l]["biases"][0].append(self.structure[l]["biases"][0][b] + (mul * (change[l]["weights"][0][b])))
    ...
    def computeLoss(self, target, result):
        ...
        loss = ((sigmoid(result[0]) - target[0]) ** 2, (sigmoid(result[1]) - target[1]) ** 2, (sigmoid(result[2]) - target[2]) ** 2, (sigmoid(result[3]) - target[3]) ** 2)
        return loss
    ...
    def replace(self, type = None, score = 0, generation = 0, partnerGenes = None, partnerBrain = None):
        ...
        if score >= self.app.scoreboard[1]["highScore"]:
...
class Player:
    ...
    def reinit(self):
        ...
        if self.id == self.app.playerCount - 1:
            self.app.game.data['racer'][0].data["angle"] = self.angle
    ...
    def updateAngle(self):
        ...
        if diff < 0:
            ...
            # self.angle = (self.angle - (18 if abs(diff) > 18 else abs(diff))) % 360
            angleChange = (self.angle - (18/self.app.frameDivisor if abs(diff) > 18/self.app.frameDivisor else abs(diff)/self.app.frameDivisor)) % 360
    ...
    def input(self):
        ...
        if self.times < len(self.app.players):
            ...
            if self.times >= len(self.app.players):
                ...
                self.times = 0
            return f'{int(p.center[0])} {int(p.center[1])} {int(p.speedVector[0])} {int(p.speedVector[1])} {int(p.angle)} {int(p.checkpoint)}'
...
class Field:
    ...
    def updateSize(self):
        ...
        self.height = self.window.height - 168
...
class App:
    ...
    targetProfiles = [((3431, 7216), (9420, 7250), (5996, 4229), (14669, 1382)),
    ...
    def indexTargets(self):
        ...
        for i, t in enumerate(self.targets):
            n = (i+1) % len(self.targets)
            p = (i-1) % len(self.targets)
            
            t.nIdx = n
            t.pIdx = p
            t.next = self.targets[n]
            t.prev = self.targets[p]
            t.path = angle(t.prev.center, t.center)
        ...
        for l in range(len(brain)):
            ...
            for wr in range(len(brain[l]["weights"])):
                ...
                for w in range(len(brain[l]["weights"][wr])):
                    ...
                    if l == 0:
                        pygame.draw.line(surface, self.colors.tgreen if brain[l]["weights"][wr][w] > 0 else self.colors.tred , nodePosition, nodePosition2, 2)
                    else:
                        pygame.draw.line(surface, self.colors.tgreen if brain[l]["weights"][wr][w] > 0 else self.colors.tred , nodePosition, nodePosition3, 2)
            ...
            if player.id == self.playerCount - 1:
                ...
                if delta < 0:
                    if abs(delta) > 18:
                        ...
                        left = (max(0.01, abs(delta)) / 18)
                        left = (left / 2) + 0.5
                elif delta > 0:
                    if abs(delta) > 18:
                        right = 1
                    else:
                        right = (max(0.01, abs(delta)) / 18)
                        right = (right / 2) + 0.5
                    
                thrust = 1 if len(str(racer.data['thrust'])) > 3 else abs(racer.data['thrust']) / 100
                boost = 1 if len(str(racer.data['thrust'])) > 3 else 0

                y = (left, right, thrust, boost)
                brain = player.agent.brain
                c = sum(player.agent.genetics.computeLoss(y, brain.results[len(brain.results) - 1][0]))
                
                print(f'{self.tick=} {c=} {y=} {player.agent.data["output"]=}')
                if c > 0.1 and ("output" in player.agent.data["previous"] and player.agent.data["output"] != player.agent.data["previous"]["output"]):
                    player.agent.genetics.backpropagation(brain.results, y, brain.inputs)
                    for p in self.players:
                        p.agent.brain.structure = player.agent.genetics.structure
                        p.moves = 0
                        p.reinit()
...
class Agent(Root):
    ...
    def __init__(self, app, laps, checkpointCount, checkpoints):
        ...
        self.genetics = Genetic(self.app, 6, 5, 2, 4, bool(not len(self.app.agents)) if self.app.save else False, None, copy.deepcopy(self.app.agents[0].genetics.structure) if bool(len(self.app.agents)) else None)
    ...
    def roundData(self, x, y, vx, vy, angle, ncid, pods):
        if ncid < 0 or ncid > len(self.app.targets):
            checkpoints = []
            for t in self.app.targets:
                checkpoints.append(point(t.x, t.y))
            
            self.update(checkpoints, "checkpoints")

        ncid = ncid % len(self.app.targets)-1 if ncid > 0 else 0
    ...
    def getMove(self):
        ...
        data = [angleDiffrence(self.data["angle"], \
        ...
        thrust = abs(math.ceil(output[2] * 100)) if not output[3] > 0.98 else "BOOST"
        if thrust == "BOOST":
        ...
        direction = self.direction(output[0], output[1])
        turn = 0
        if direction < 0:
            turn = ((output[0] - 0.5) * 2)
        if direction > 0:
        ...
        #     print(f'{output[0]=} {output[1]=} {direction=} {self.data["angle"]=}')
        
        x = int(int(thrust * 200) * (math.cos((self.data["angle"] + ((min(1, 18 * turn)) * direction)) * (math.pi / 180)))) + self.data["location"][0]
        y = int(int(thrust * 200) * (math.sin((self.data["angle"] + ((min(1, 18 * turn)) * direction)) * (math.pi / 180)))) + self.data["location"][1]
...
if __name__ == "__main__" :
    # print(sys.argv[1])
    theApp = App(random.randint(3, 8), int(sys.argv[1]) if len(sys.argv) > 1 else None, 2, save = False)
    theApp.on_execute()